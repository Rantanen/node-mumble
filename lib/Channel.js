...
Channel.prototype.join = function() {
    this.client.connection.sendMessage( 'UserState', { session: this.client.user.session, actor: this.client.user.session, channelId: this.id });
};
...
Channel.prototype.sendMessage = function(message) {
    this.client.sendMessage( message, { channelId: [ this.id ] });
};
...
Channel.prototype._removeChild = function(channel) {
    var index = this.children.indxOf( channel );
    if( index !== -1 )
