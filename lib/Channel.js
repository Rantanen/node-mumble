 ...
 */
Channel.prototype.sendMessage = function( message, broadcast ) {
    if (broadcast) this.client.sendMessage( message, { channel_id: [ this.id ], tree_id: [ this.id ] } );
    else this.client.sendMessage( message, { channel_id: [ this.id ] } );
};
...
Channel.prototype.getPermissions = function( callback ) {
    ...
    if( this.permissions ) {
        process.nextTick( callback.bind( null, null, this.permissions ) );
        return;
    }
 ...
 * @param {Permissions} query - The new permissions
 */
...
Channel.prototype._checkParent = function( data ) {
    ...
    if( this.parent ) {
        ...
        // parent can be 0. Therefor we need to check for undefined
