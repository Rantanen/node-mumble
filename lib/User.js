...
User.prototype.moveToChannel = function(channel) {
    ...
    this.wrapper.connection.sendMessage( 'UserState', { session: this.session, actor: this.wrapper.user.session, channelId: id });
};
...
User.prototype.sendMessage = function(message) {
    this.wrapper.sendMessage( message, { session: [ this.session ] } );
};
...
User.prototype._checkChangeChannel = function( data ) {
    ...
    var actor = this.wrapper.userBySession( data.actor );
