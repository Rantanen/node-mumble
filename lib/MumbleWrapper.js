...
var MumbleWrapper = function(connection) {
    ...
    this.user = undefined;
    this._gotServerSync;
    this._gotInitialPermissionQuery;
...
MumbleWrapper.prototype._checkReady = function() {
    if(this._gotInitialPermissionQuery && this._gotServerSync && this._gotInitialPing && !this.ready) {
        this.ready = true;
        this.emit('ready');
    }
};
...
MumbleWrapper.prototype._permissionQuery = function(data) {
    //TODO: Do something with the data
    this._gotInitialPermissionQuery = true;
    this._checkReady();
};
...
MumbleWrapper.prototype._newChannel = function(data) {
    ...
    this._wrapEvents( channel, 'channel', [
        'rename', 'links-add', 'links-remove', 'move', 'remove' ] );
