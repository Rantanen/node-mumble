...
var MumbleWrapper = function(connection) {
	...
	connection.on('channelRemove', function(data) { thisMumbleWrapper._channelRemove(data); });
...
MumbleWrapper.prototype._channelRemove = function(data) {
	if(data.channelId) {
		...
		channel._detach();
		delete this.channels[data.channelId];
	}
};
...
MumbleWrapper.prototype._newChannel = function(data) {
	...
	channel.on('remove', function() { thisMumbleWrapper.emit('channel-remove', channel); });
	if(this.ready) {
