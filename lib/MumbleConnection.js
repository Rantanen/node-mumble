...
var MumbleConnection = function (socket, options) {
    ...
    this.opusEnabled = false; // Only enable when server tells us to.
...
MumbleConnection.prototype.sendVoiceFrame = function( frame ) {
    ...
    var encoded;
    if(this.opusEnabled) {
        encoded = this.opusEncoder.encode( frame, this.FRAME_SIZE );
    }
    else {
        encoded = this.celtEncoder.encode( frame );
    }
...
MumbleConnection.prototype._onCodecVersion = function( codecVersion ) {
    if(this.opusEnabled && ! codecVersion.opus) { // Server switched from opus to celt
        for(var key in this.users) {
            var user = this.users[key];
            if( user ){
                user.decoder = new CeltEncoder( this.SAMPLING_RATE, this.FRAME_SIZE );
            }
        }
    }
    if(!this.opusEnabled && codecVersion.opus && this.options.opus) { // Server switched from celt to opus
        for(var key in this.users) {
            var user = this.users[key];
            if( user ){
                user.decoder = new OpusEncoder( this.SAMPLING_RATE );
            }
        }
    }
    if(codecVersion.opus) {
        this.opusEnabled = this.options.opus;
    }
    else {
...
MumbleConnection.prototype._onUserState = function ( userState ) {
    ...
    if( !user ) {
        var decoder;
        if(this.opusEnabled) {
            decoder = new OpusEncoder( this.SAMPLING_RATE );
        }
        else {
    ...
    while( this.lastProcess + this.FRAME_LENGTH < Date.now() )
    {
        ...
        if( packets.length > 0 && (
            ...
            for( var f in packets ) {
                var data;
                if(this.opusEnabled) {
                    data = packets[f].decoder.decode( packets[f].frame, this.FRAME_SIZE );
                }
                else {
