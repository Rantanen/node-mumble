...
MumbleConnection.prototype.sendVoiceFrame = function( frame ) {
    ...
    // If frame is empty, we got nothign to send.
    if( frame.length === 0 ) {
        return;
    }
    ...
    this.sendEncodedFrame( encoded, this.codec );
 ...
 **/
MumbleConnection.prototype.sendEncodedFrame = function ( packets, codec ) {
    ...
    while( this.lastProcess + this.FRAME_LENGTH < Date.now() ) {
        ...
        if( decoded.length > 0 ) {
            ...
            Object.keys( decodedUser ).forEach( function( key ) {
                self.emit('voice-user-' + key, decodedUser[ key ]);
            });
