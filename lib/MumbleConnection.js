...
MumbleConnection.prototype.initialize = function () {
    this.sendMessage('Version', { version: encodeVersion(1, 2, 7), release: 'Node.js-client', os: 'Node.js', osVersion: process.version });
};
 ...
 *
 * @param {Number} whisperId - Optional whisper group ID.
 **/
MumbleConnection.prototype.inputStream = function ( whisperId ) {
    var stream = new MumbleInputStream( this, { whisperId: whisperId } );
    return stream;
};
 ...
 * @param {Number} whisperTarget - Optional whisper target ID.
 **/
MumbleConnection.prototype.sendVoiceFrame = function( frame, whisperTarget ) {
    ...
    this.sendEncodedFrame( encoded, this.codec, whisperTarget );
 ...
 * @param {Number} whisperTarget - Optional whisper target ID.
 **/
MumbleConnection.prototype.sendEncodedFrame = function ( packets, codec, whisperTarget ) {
...
MumbleConnection.prototype._onUDPTunnel = function( data ) {
    ...
    var type = data[0];
    if( type === 0x00 ) {
        type = MumbleConnection.codec.Celt;
    } else if( type === 0x80 ) {
        type = MumbleConnection.codec.Opus;
    }
    ...
    while( packet.length > 0 ) {
        ...
        // Audio frame header.
        var header, length, frame, terminator, headerLength;
        if( type === MumbleConnection.codec.Celt ) {

            // Celt header is two bytes.
            header = packet[0];
            length = header & 0x7F;
            terminator = !( header & 0x80 );
            headerLength = 1;
            ...
            terminator = header & 0x2000;
        }
        if( length === 0 ) { break; }
        frame = packet.slice( headerLength, length + headerLength );
        ...
        packet = packet.slice( length + headerLength );
        sequence++;
    }
};
