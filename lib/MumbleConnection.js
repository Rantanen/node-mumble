...
MumbleConnection.prototype.authenticate = function( name, password ) {
    ...
    console.dir( 'Celt versions: ' + this.options.celtVersions );
    this.authSent = true;
};
...
MumbleConnection.prototype.joinPath = function ( path ) {
    ...
    console.dir( path );
    for( var i in path ) {
        console.log( 'Current channel: ' + channel.name + ' (' + channel.channelId + ')' );
        if( path[i] === '' ) { continue; }
    ...
    console.log( 'Final channel: ' + channel.name + ' (' + channel.channelId + ')' );
    ...
    while( this.lastProcess + this.FRAME_LENGTH < Date.now() )
    {
        ...
        for( var i in this.users ) {
            ...
            if( frame.data ) {
                ...
                packet.data = user.lastFrame.data;
        ...
        if( packets.length > 0 && (
            ...
            for( var f in packets ) {
                decoded.push({
                    data: packets[f].decoder.decode( packets[f].data ),
            ...
            if( this.listeners('voice').length > 0 ) {
                ...
                if( decoded.length === 1 ) {
                    ...
                    for( var i = 0; i < this.FRAME_SIZE; i++ ) {
                        var sum = 0;
                        for( var d in decoded ) { sum += decoded[d].data.readInt16LE( i*2 ); }
